...
namespace polyline {
	...
	public:
		...
		Polyline(const Point<T>& a) { //point coordinate
			_data = new Point<T>;
			_data[0] = Point<T>(a._x, a._y);
			_size = 1;
		}

		Polyline(size_t size) { //number of points
		...
		Polyline(int count, T x1, T y1, T x2, T y2) { //range of points
		...
		Polyline<T> operator+(Polyline<T>& a) { //adding two polylines
		...
		void push_back(Point<T>& point) { //adding a vertex to the end of a polyline
		...
		void push_front(Point<T>& point) { //inserting a vertex at the beginning of a polyline
			auto temp = new Point<T>[_size + 1];
		...
		float len() { //calculating the length of a polyline
			float length = 0;
			for (int i = 1; i<_size; ++i)
	...
	template <typename T>
	void drawing_polygon(double a, int n) {
		Point<T> x;
		Polyline line(x);
		for (int i = 0; i < n; ++i) {
			Point<T> vertex;
			double angle = PI / n;
			/*double tmp = cos((PI) / 2);*/
			double cos_angle = cos(angle/180 +2.0 * PI *i / n);
			vertex._x = a * cos_angle;
			double sin_angle = sin(angle/180 + 2.0 * PI * i / n);
			/*double tmp2 = sin((PI) / 2);*/
			vertex._y = a * sin_angle;
